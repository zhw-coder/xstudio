...
use ai::{
    agent::{
        ...
        harness::{
            ...
            types::{
                AfterProviderResponseHook, AgentHarnessEvent, AgentHarnessOwnEvent, AgentHarnessResources,
                AgentHarnessStreamOptions, BeforeProviderPayloadHook, SessionCreateStorageOptions, SessionStorage,
            },
        },
        types::{AgentError, MHook},
    },
    model::{
        types::{Auth, AuthProvider, ImageContent, Model, ModelCost},
        ThinkingLevel,
    },
};
...
impl<'a> MHook<AgentHarnessEvent<'a>, ()> for HarnessLoggingListener {
    async fn execute(&self, event: &mut AgentHarnessEvent<'a>) -> Result<(), AgentError> {
        let name = match event {
            ...
            AgentHarnessEvent::Harness(harness_event) => {
                let name = match harness_event {
                    AgentHarnessOwnEvent::BeforeAgentStart { prompt, system_prompt, .. } => {
                    ...
                    AgentHarnessOwnEvent::AfterProviderResponse { status, headers } => {
                        format!("harness:after_provider_response status={status} headers={headers:?}")
                    }
                    AgentHarnessOwnEvent::SavePoint { had_pending_mutations } => {
                    ...
                    AgentHarnessOwnEvent::QueueUpdate { steer, follow_up, next_turn } => {
                        format!(
                    ...
                    AgentHarnessOwnEvent::Abort { cleared_steer, cleared_follow_up } => {
                        format!(
                    ...
                    AgentHarnessOwnEvent::ModelSelect { model, previous_model, source } => {
                        format!(
                    ...
                    AgentHarnessOwnEvent::ThinkingLevelSelect { level, previous_level } => {
                        format!("harness:thinking_level_select level={level:?} previous={previous_level:?}")
                    }
                    AgentHarnessOwnEvent::ResourcesUpdate { resources, previous_resources } => {
                        format!(
                    ...
                    AgentHarnessOwnEvent::ToolCall { tool_call_id, tool_name, .. } => {
                        format!("harness:tool_call id={tool_call_id} name={tool_name}")
                    }
                    AgentHarnessOwnEvent::ToolResult { tool_call_id, tool_name, is_error, .. } => {
        ...
        self.logs.lock().expect("harness logging lock poisoned").push(name);
        Ok(())
    }
}
...
impl<'a> MHook<AgentHarnessOwnEvent<'a>, Option<Value>> for LoggingPayloadHook {
    async fn execute(&self, event: &mut AgentHarnessOwnEvent<'a>) -> Result<Option<Value>, AgentError> {
        if let AgentHarnessOwnEvent::BeforeProviderPayload { model, payload } = event {
            ...
            println!("[hook:before_provider_payload] model={} payload={}", model.id, payload_text);
            println!("[hook:before_provider_payload] model_name={} provider={}", model.name, model.provider);
            self.logs.lock().expect("payload hook lock poisoned").push("payload".to_string());
        }
        Ok(None)
    }
}
...
impl<'a> MHook<AgentHarnessOwnEvent<'a>, ()> for LoggingResponseHook {
    async fn execute(&self, event: &mut AgentHarnessOwnEvent<'a>) -> Result<(), AgentError> {
        if let AgentHarnessOwnEvent::AfterProviderResponse { status, headers } = event {
            ...
            self.logs.lock().expect("response hook lock poisoned").push("response".to_string());
        }
        Ok(())
    }
}
...
async fn harness_agent_sqlite_storage_with_event_logging() {
    ...
    let model_id = std::env::var("OPENAI_TEST_MODEL_ID").unwrap_or_else(|_| "gpt-5.6-terra".to_string());
    ...
    let temp_dir = data_dir; //.join(format!("harness-test-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&temp_dir).expect("should create data dir");
    let session_file = temp_dir.join(format!("harness-test-{}.sqlite", uuid::Uuid::new_v4()));
    ...
    // 构建 Harness 事件监听器。
    let harness_listener = Arc::new(HarnessLoggingListener { logs: harness_logs.clone() });

    // 构建 Provider hooks。
    let payload_hook: Box<BeforeProviderPayloadHook> = Box::new(LoggingPayloadHook { logs: payload_logs.clone() });
    let response_hook: Box<AfterProviderResponseHook> = Box::new(LoggingResponseHook { logs: response_logs.clone() });

    // 构建 stream options。
    let stream_options = AgentHarnessStreamOptions {
        headers: {
            let mut headers = HashMap::new();
            headers.insert("x-test-case".to_string(), "harness_agent_sqlite_storage".to_string());
            headers
        },
        ..Default::default()
    };
    ...
    let harness = AgentHarness::new(AgentHarnessOptions {
        ...
        stream_options,
        system_prompt: "你是一个高级教授，用专业回答问题。".to_string(),
    ...
    harness.append_session_name("自我介绍").await.expect("harness should construct");
    ...
    // 执行 prompt。
    let result = harness.prompt("你好，请出一道复杂的中学奥数题", None).await;
    ...
    match result {
        Ok(message) => {
            ...
            println!("[harness-result] model={} provider={}", message.model, message.provider);
        }
        Err(error) => {
    ...
    assert!(
        harness_events.iter().any(|event| event.contains("before_agent_start")),
        "expected before_agent_start event"
    );
    assert!(
        harness_events.iter().any(|event| event.contains("before_provider_request")),
        "expected before_provider_request event"
    );
    assert!(harness_events.iter().any(|event| event.contains("save_point")), "expected save_point event");
    assert!(harness_events.iter().any(|event| event.contains("settled")), "expected settled event");
    ...
    assert!(session_file.exists(), "expected sqlite session file to exist");
    let metadata = std::fs::metadata(&session_file).expect("should read sqlite session file metadata");
    assert!(metadata.len() > 0, "expected sqlite session file to be non-empty");
    println!("[test-validation] session file has {} bytes", metadata.len());
